...
func handleSubscriptionVless(w http.ResponseWriter, r *http.Request) {
	...
	for _, n := range nodes {
		...
		// vless://{uuid}@{host}:443?type=xhttp&security=tls&sni={host}&path=%2F{path}#{name}
		params := url.Values{}
...
// 要求 Mihomo 1.18.x+ （支持 network: xhttp）
...
`, yamlStr(name), host, n.XrayUUID, host, xrayPath, host)
	...
	fmt.Fprintln(yaml, "# Generated by hive-registry  (requires Mihomo 1.18.x+)")
	fmt.Fprintln(yaml, "")
